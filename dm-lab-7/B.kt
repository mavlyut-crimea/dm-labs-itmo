...
class Queue(private val n: Int, private val a: IntArray = IntArray(n) { it + 1 }) {
    private var head = 0
...
fun main() {
    ...
    val q = Queue(n, Array(n) { Pair(g[it + 1].size, it + 1) }.sortedWith { i, j ->
        if (i.first == j.first)
            j.second - i.second
        else
            i.first - j.first
    ...
    for (k in 0 .. n * (n - 1)) {
        if (!g[q[0]][q[1]]) {
            i = 2
            while (i < n && (!g[q[0]][q[i]] || !g[q[1]][q[i + 1]]))
                i++
            if (i == n) {
                i = 2
                while (!g[q[0]][q[i]])
